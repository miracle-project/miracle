...
#if defined(NDEBUG)
# error "Miracle cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf39a29091761e1ff58dbdc188b713e3f90368460f6399cda457c795f024b1788");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Miracle: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Miracle Signed Message:\n";
...
{
    // Miracle: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 0 * COIN;
		...
		if (nHeight <= 100) { nSubsidy = 100000 * COIN; }
		else if (nHeight <= 650100) { nSubsidy = 100 * COIN; }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Miracle: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Miracle: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "MiracleCoin the new way to financing the global world";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("12345f6508c5a231e287e5cd08d8188452ad06f875093d47bac77ff8b5246bb6e665afcc42891d6453abf0250ac91b40e38787ee7ffeefbfbc3c651f3353767890") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1541551495;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 5709749;

        if (fTestNet)
        {
            block.nTime    = 1541551000;
            block.nNonce   = 0;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x3b92cbd3e871b85fad01743c3ac1f113de806bbee8b7be0c5953c318ea07ee94"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc1, 0xb3, 0xdc }; // Miracle: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MiracleMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MiracleMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MiracleMiner(CWallet *pwallet)
{
    printf("MiracleMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("miracle-miner");
    ...
    try { loop {
        ...
        printf("Running MiracleMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MiracleMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MiracleMiner, pwallet));
}
